...
#' 
#' @description For each point in the trajectory, a circle of radius R is drawn around that point.
...
#' 
#' @param xyt the trajectory data with three columns, x the x-coordinate, 
#' y the y-cordinate, and t the time 
#' @param radius radius to use in units of the location data to detect recursions
#' 
...
getRecursions = function(xyt, radius)
{
		...
		for (j in 2:n) 
		{
			if (!inRadius[j]) # is location outside radius?
			{
				if (stillInside) 
				{
					...
					stats = data.frame(z[i], i, revisits[i], radiusEntranceTime, radiusExitTime, 
								 radiusExitTime - radiusEntranceTime, timeSinceLastVisit)
	...
	names(revisitStats) = c("focalCoord", "coordIdx", "visitIdx", "entranceTime", "exitTime", "timeInside", "timeSinceLastVisit")
	return(list(revisits = revisits, dists = distMatrix, revisitStats = revisitStats))
}
